-- =========================================================
-- Fix: Preserve renegotiated parcelas during null-anchor cleanup
--
-- When entrada_vencimento or primeira_parcela_vencimento is cleared
-- (set to null) on a tratamento, recalcular_parcelas_tratamento wipes
-- ALL parcelas of that tipo. Previously this delete did not exclude
-- renegotiated rows, meaning clearing a due date field could silently
-- delete a renegotiated parcela with zero warning. This contradicts
-- the feature's guarantee that renegotiated parcelas are never deleted
-- by recalculation.
--
-- This migration adds `and renegociada = false` to both null-anchor
-- DELETE statements, matching the guard already present on the two
-- count-shrink DELETE statements below them.
-- =========================================================
...
begin
  ...
  if v_tratamento.entrada_vencimento is null then
    delete from parcelas where tratamento_id = p_tratamento_id and tipo = 'entrada' and renegociada = false;
  else
  ...
  if v_tratamento.primeira_parcela_vencimento is null then
    delete from parcelas where tratamento_id = p_tratamento_id and tipo = 'tratamento' and renegociada = false;
  else
